...
class FoodSnapperScreen extends StatefulWidget {
  ...
  const FoodSnapperScreen({super.key, required this.onAdd, required this.onCancel});
...
class FoodSnapperScreenState extends State<FoodSnapperScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _analyzeImage() async {
    ...
    try {
      final results = await _geminiService.analyzeFood(_imageBytes!).timeout(
        ...
        onTimeout: () => throw Exception("Timeout 60s. Vui lòng thử lại."),
      );
      setState(() {
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          _nutrientStat('Carb', '${_analysisResults!['carbs']}', 'g', const Color(0xFF0277BD)),
                        ],
                      ),
                    ),
                    const SizedBox(height: 30),
  ...
  Widget _nutrientStat(String l, String v, String u, Color c) {
    return Column(
      children: [
        Text(l, style: const TextStyle(color: Color(0xFF5A5F5D), fontSize: 12)),
        const SizedBox(height: 4),
        Text(v, style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.w900, color: c)),
        Text(u, style: TextStyle(fontSize: 10, color: c.withOpacity(0.7), fontWeight: FontWeight.bold)),
      ],
    );
  }
  ...
  Widget _buildLoadingOverlay() {
    return Container(color: Colors.black54, child: const Center(child: CircularProgressIndicator(color: Color(0xFF8CE3BE))));
  }
}
